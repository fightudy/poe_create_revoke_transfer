...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn transfer_claim(origin: OriginFor<T>, claim: T::Hash, new_owner: T::AccountId) -> DispatchResult {
            ...
            Claims::<T>::try_mutate(&claim, |claim_details| {
                if let Some((ref mut owner, _)) = claim_details {
                    // Verify that sender of the current call is the claim owner.
                    ensure!(*owner == sender, Error::<T>::NotClaimOwner);

                    // Update the owner of the claim.
                    *owner = new_owner.clone();

                    // Emit an event that the claim was transferred.
                    Self::deposit_event(Event::ClaimTransferred { old_owner: sender, new_owner, claim });

                    Ok(())
                } else {
                    Err(Error::<T>::NoSuchClaim.into())
                }
            })
        }
    }
